...
const GenerateImages = () => {
  ...
  if (error.response?.status === 402) {
    toast.error("API quota exceeded. Upgrade your plan.");
  } else {
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} action="" className='w-full max-w-lg p-4 bg-white rounded-lg border- border-gray-200'>
        <div className='flex items-center gap-3'>
          <Sparkles className='w-6 text-[#0580a6]'/>
        ...
        placeholder='Describe what you want to see in the image..'required></textarea>
        ...
        {imageStyle.map((item)=>(
          <span onClick={()=> setSelectedStyle(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer
             ${selectedStyle === item ? 'bg-[#e8fffc] text-[#0580a6]' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span>
        ))}
        </div>
        <div className='my-6 flex items-center gap-2'>
          <label className='relative cursor-pointer '>
            ...
            <div className='w-9 h-5 bg-slate-300 rounded-full peer-checked:bg-[#0580a6] transition'></div>
        ...
        <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#0580a6] to-[#54fae9] 
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
        ...
        <Image className='w-5 h-5 text-[#0580a6]'></Image>
        <h1>Generated Image</h1>
         ...
         {!content ? (
          ...
          <div>
            ...
            <p>Enter a topppic and click "Generate image" to get started</p>
          </div>
        </div>
         ):
         (
